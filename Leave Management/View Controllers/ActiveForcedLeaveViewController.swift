//
//  ActiveForcedLeaveViewController.swift
//  Leave Management
//
...
class ActiveForcedLeaveViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        refreshControl.attributedTitle = NSAttributedString(string: "Fetching active compulsory leaves")
    ...
    func loadLeaves() {
        ...
        let baseURL = "http://portal.adriankenya.work/api/active/forced/leaves"
        if let _token = token {
...
//                                self.present(alert, animated: true, completion: nil)
                                ...
                                if self.activeLeaves.count == 0 {
                                    ...
                                    messageLabel.text = "No active compulsory leaves found"
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "activeForcedLeaveCell", for: indexPath)
        
        // Configure the cell...
        guard let leaveCell = cell as? ActiveForcedLeaveTableViewCell else {
            return cell
        }
        let request = activeLeaves[indexPath.row]
        leaveCell.reason.text = request.reason
    ...
    */
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let identifier = segue.identifier {
            switch identifier {
            case "leaveDetail" :
                if let cell = sender as? ActiveForcedLeaveTableViewCell, let indexPath = tableView.indexPath(for: cell), let activeLeaveDetailMVC = segue.destination as? ActiveForcedLeaveDetailViewController {
                    
                    activeLeaveDetailMVC.forcedLeave = activeLeaves[indexPath.row]
                }
